$(document).ready(function() {
	var gTableCaptionText = $("#goodTable caption").text();
		...
		format: function(table) {
			...
			jQuery('tr', table).each( function(e) {
				...
				$('th.'+table.config.cssHeader, this).each( function(e) {
					...
					$("#goodTable caption").text(gTableCaptionText); // modify
				...
				$(this).find( 'th.'+table.config.cssAsc).each( function(e) {
					...
					$("#goodTable caption").text(gTableCaptionText + ': Sorted By ' + headerText + ' Descending ');
				...
				$('th.'+table.config.cssDesc, this).each( function(e) {
					...
					var headerText = $(this).text();
					$("#goodTable caption").text(gTableCaptionText + ': Sorted By ' + headerText + ' Ascending ');
	...
	$("#goodTable thead th").attr("role", "columnheader").attr("scope", "col");
	$("#goodTable tbody th").attr("role", "rowheader").attr("scope", "row");
	...
	$("#goodTable th.header").attr("tabindex", "0");

	// assign role of gridcell on all data cells
	$("#goodTable tbody td").attr("role", "gridcell");

	$('#goodTable caption').attr({
		role: 'log',
		'aria-live': 'assertive',
		'aria-relevant': 'all',
		'aria-atomic': 'true'
